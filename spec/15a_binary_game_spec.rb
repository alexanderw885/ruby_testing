...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    let(:min) { 1 }
    let(:max) { 10 }
    subject(:game_verify) { described_class.new(min, max) }

    matcher :be_in_range do
      match { |val| val.between?(min, max) }
    end
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      subject(:game_six_hund) { described_class.new(100, 600) }
      it 'returns 9' do
        max_guesses = game_six_hund.maximum_guesses
  ...
  describe '#display_turn_order' do
    ...
    let(:search_display) { instance_double(BinarySearch) }
    subject(:game_display) { described_class.new(1, 10, search_display) }
    ...
    it 'increases guess_count by one' do
      expect { game_display.display_turn_order(search_display) }
        .to change { game_display.instance_variable_get(:@guess_count) }
        .by(1)
    end
