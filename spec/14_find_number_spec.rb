...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:correct_random) { double('random_number', value: 8) }
      subject(:complete_game) { described_class.new(0, 21, correct_random, 8) }
    ...
    context 'when guess and random_number are not equal' do
      let(:wrong_random) { double('random_number', value: 8) }
      subject(:not_over) { described_class.new(0, 21, wrong_random, 6) }
      it 'is not game over' do
        over = not_over.game_over?
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:min_max) { described_class.new(5, 8, number_range, 7) }
      it 'updates min to the same value as max' do
        min_max.update_range
        expect(min_max.min).to eq(8)
      end

      it 'does not update max' do
        expect { min_max.update_range }.not_to change { min_max.max }.from(8)
      end
    end
  end
end
